    ...
    <div className="relative px-5 py-10 md:py-20 overflow-hidden">
      <div className="mx-auto max-w-320 space-y-8 md:space-y-14">
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-12">
          ...
          <div className="flex flex-row md:flex-col gap-3 md:min-w-[180px]">
            {days.map(({ day, label, date, suffix, month }) => (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border transition-all text-left font-poppins
                  ${activeDay === day
          ...
          {/* Events List — wave sits beneath this column */}
          <div className="relative flex-1 flex flex-col gap-4 pb-16">

            {scheduleData[activeDay].map((event) => (
              <div key={event.id} className="relative z-10 bg-[#f7f7f7] rounded-xl px-6 py-5 space-y-3">
                ...
                <p className="font-neue-machina font-bold text-xl text-semantic-text-primary">
                  {event.title}
                </p>
            ...
            {/* Wave sits inside the events column, behind the cards */}
